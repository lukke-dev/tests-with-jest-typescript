...
const createSutWithProducts = () => {
  ...
  const cartItem = createCartItem('camiseta', 99.9);
  const cartItem2 = createCartItem('caderno', 20.9);
...
describe('ShoppingCart', () => {
  ...
  it('should test total and totalWithDiscount', () => {
    const { sut } = createSutWithProducts();
    expect(sut.total()).toBe(120.8);
    expect(sut.totalWithDiscount()).toBe(120.8);
  });
